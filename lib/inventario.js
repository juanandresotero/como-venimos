...
export const TIPOS_DE_AMBIENTE = [
  ...
  /* LO QUE DEJA EL PROPIETARIO. No es un ambiente: son las cosas que quedan en la casa —una
     heladera, un lavarropas, las cortinas— y que el día que el inquilino se va hay que
     devolver. Juan: "son objetos que quedan en la propiedad que la dueña deja. funciona todo
     igual, con fotos, detalles, cantidades y demás".

     ARRANCA VACIO, y es a proposito: no hay dos propiedades que dejen las mismas cosas, y la
     regla de estas plantillas es que salen de SU inventario, no de lo que yo imagine que
     deberia haber. Lo que se repita, se agrega despues con su lista en la mano. */
  { clave: "objetos", nombre: "Objetos que deja el propietario", items: [] },
