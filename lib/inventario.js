...
export const ESTADOS = [
  /* `nombre` es lo que se elige en la pantalla y `texto` lo que sale impreso. Son distintos a
     proposito: en el telefono el desplegable compite por el ancho con el nombre de la cosa, y
     con "Buen estado – sin detalles" adentro no entraba ni "Aire acondicionado" al lado. En el
     papel, en cambio, manda el vocabulario de Juan, palabra por palabra. */
  { clave: "bien", nombre: "Bien", texto: "Buen estado – sin detalles" },
  { clave: "perfecto", nombre: "Perfecto", texto: "Perfecto – sin detalles" },
  { clave: "detalles", nombre: "Detalles", texto: "Buen estado – con detalles" },
  { clave: "malo", nombre: "Problema", texto: "" },
...
export function comoSeLee(item) {
  ...
  const base = (ESTADO[item.estado] || ESTADO[POR_DEFECTO]).texto;
  const detalle = (item.detalle || "").trim();
  if (!detalle) return base || "Con problemas";
...
export function comoVa(inv) {
  ...
  return {
    ...
    items: usados.length,
    conDetalle: usados.filter((i) => i.estado === "detalles" || i.estado === "malo"
