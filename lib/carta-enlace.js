...
export async function aEnlace(base, estado) {
  /* Lo que esta vacio NO viaja. Cada nombre de casilla y cada llave suelta son
     caracteres en un enlace que el usuario ve entero en WhatsApp, y un enlace largo da
     desconfianza. Tampoco viaja su telefono: el boton de mandar ya no elige contacto. */
  const valores = {};
  for (const [clave, valor] of Object.entries(estado.valores || {})) {
    if (valor !== null && valor !== undefined && valor !== "") valores[clave] = valor;
  }
  const cabeza = new TextEncoder().encode(JSON.stringify({
    v: VERSION,
    valores,
    ...(estado.quitadas && estado.quitadas.length ? { quitadas: estado.quitadas } : {}),
    turno: estado.turno || "comprador",
    ...(estado.agente ? { agente: estado.agente } : {}),
  }));
