...
export function capas(negocios, cartera, ajustes, anio) {
  ...
  return {
    ...
    capa2,
    capa3,
    avanzado: avanzado(enCurso, cartera, yaContadas, r),
    ratios: r,
    total: {
...
/* "Si cierro todo lo que está en negociación y reservado."

   Es distinto de la capa 3: acá NO se aplica la probabilidad de cierre. La pregunta no es
   "cuánto vale esto hoy" sino "cuánto cobro si cierra todo", así que va al 100%. Y deja
   afuera lo que está apenas publicado, que todavía no se movió. */
const ESTADOS_AVANZADOS = new Set(["en_negociacion", "reservada"]);

function avanzado(enCurso, cartera, yaContadas, ratios) {
  const detalle = enCurso.map((n) => ({
    origen: "negocio",
  ...
  for (const propiedad of Object.values(cartera || {})) {
    if (!propiedad.activa || !propiedad.usar_en_proyeccion) continue;
    if (!ESTADOS_AVANZADOS.has(propiedad.estado)) continue;
    if (yaContadas.has(propiedad.entity_id)) continue;
    const ratio = propiedad.operacion === "alquiler" ? ratios.alquiler : ratios.venta;
    detalle.push({
      origen: "propiedad",
      entity_id: propiedad.entity_id,
      direccion: propiedad.direccion,
      estado: propiedad.estado.replace("_", " "),
      facturacion: propiedad.precio * ratio.fact,
      ganancia: propiedad.precio * ratio.gan,
    });
  }
  ...
  return {
    ...
    facturacion: detalle.reduce((t, x) => t + x.facturacion, 0),
    ganancia: detalle.reduce((t, x) => t + x.ganancia, 0),
    detalle,
  };
}
