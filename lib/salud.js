...
import { calcular, CAIDO, momentoDelNegocio } from "./motor.js";
...
export function capas(negocios, cartera, ajustes, anio) {
  ...
  /* LA MISMA VENTA NO SE CUENTA DOS VECES. RE/MAX tarda dias en bajar un aviso: la propiedad
     puede figurar reservada en el portal cuando la escritura ya se firmo y esa plata ya esta
     en lo cobrado. Estanislao Vega 3900 estaba en los dos lados —cobrada de verdad, y otra
     vez estimada en lo reservado— y Juan lo vio en los numeros (2026-09-24).

     Sale de la proyeccion SOLO si no quedo ningun negocio abierto sobre esa propiedad: un
     alquiler que rota tiene el del año pasado cobrado y el nuevo en marcha, y ese cuenta. Y
     solo mientras la propiedad no haya vuelto a publicarse: ahi arranca otra vuelta. */
  const yaCobrada = new Set();
  for (const n of negocios || []) {
    if (!n.entity_id_cartera || n.estado !== "cerrado" || !n.fecha_fin) continue;
    const suya = (cartera || {})[n.entity_id_cartera];
    if (!suya || GRUPOS_CARTERA[suya.estado] === "publicado") continue;
    const arranco = suya.fecha_reservada || suya.fecha_negociacion || "";
    if (n.fecha_fin >= arranco) yaCobrada.add(n.entity_id_cartera);
  }
  ...
  for (const propiedad of Object.values(cartera || {})) {
    ...
    if (!grupo) continue;
    if (yaCobrada.has(propiedad.entity_id) && !negocioDe.has(propiedad.entity_id)) continue;
    ...
    grupos[grupo].push({
      ...
      // Para poder abrir el negocio desde la lista de lo encaminado, cuando ya existe.
      negocio_id: suNegocio ? suNegocio.id : null,
  ...
  /* Un negocio en curso sin propiedad en la cartera —una busqueda, una referida— igual esta
     en marcha, y va a la capa del MOMENTO EN QUE ESTA: con el boleto firmado es una reserva,
     no una negociacion. Antes caian todas en negociacion porque la app miraba el estado de la
     propiedad, y en una busqueda esa propiedad no es suya.

     Juan lo conto al reves y tenia razon: "dice que tengo 3 en negociacion y 3 reservadas
     (hay 5 reservadas y una en negociacion hoy)". Las dos que sobraban eran Calle 6 y Blanca
     del Tabare, con el boleto ya firmado. */
  for (const n of enCurso) {
    if (n.entity_id_cartera && (cartera || {})[n.entity_id_cartera]) continue;
    const reservada = momentoDelNegocio(n) === "reservada";
    grupos[reservada ? "reservado" : "negociacion"].push({
      negocio_id: n.id,
      direccion: n.direccion || n.barrio || n.id,
      estado: reservada ? "reservada" : "en_negociacion",
      precio: n.precio_operacion,
      probabilidad: probabilidades[reservada ? "reservada" : "en_negociacion"] || 0,
      estimado: false,
