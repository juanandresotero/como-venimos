...
export const AGENTES = [
  ...
  "Ofi Único",
  "Otra Oficina",
  /* Fuera de RE/MAX. Pasa en las busquedas: el aviso lo tiene una inmobiliaria de afuera. */
  "Inmobiliaria exterior",
  /* Y a veces no hay agente del otro lado: la vende el propio dueño. Lo marco Juan, que es
     el que hace las busquedas y se encuentra con ese caso. */
  "Dueño vende",
];
...
export const AGENTES_QUE_LLEVAN_NOMBRE = new Set(
  ["Team", "Ofi Único", "Otra Oficina", "Inmobiliaria exterior"]);

/* Cómo llegó el negocio. Es una sola pregunta, y de la respuesta sale sola la regla de
   comisión: no hay que cargar las dos cosas. */
...
export const EXPLICACION_ORIGEN = {
  ...
  "Ref. B.d.r.": "Alguien de tu base te lo refirió",
};

/* LOS ORIGENES QUE SON UN REFERIDO, aunque el nombre no empiece con "Ref.".

   Sirven para una sola cosa: saber que ese negocio llego referido, y con eso decidir el
   regimen de comision. Antes tambien encendian un campo "¿quien en concreto?" al lado del
   origen; ese campo se saco. Juan: "en ese lugar de dónde me llegó era la respuesta y no
   quería más específico que las opciones desplegables que tenemos". En 87 negocios no lo
   habia llenado nunca.

   El vocabulario viejo del Excel esta incluido a proposito: si no estuviera, al revisar un
   negocio importado su regimen cambiaria solo y le cambiaria la plata. */
export const ORIGENES_QUE_SON_REFERIDO = new Set([
  "Ref. B.d.r.",
  "Ref. Team",
  "Ref. Único",
  "Ref. Remax",
...
/* El Excel usaba otros nombres para lo mismo. Se traducen al vocabulario de hoy: si no,
   el desplegable no encuentra el valor y aparece VACIO, aunque el dato esté cargado.

   Ninguna de estas traducciones cambia la regla de comisión — las dos formas de escribir
   cada origen caen en el mismo régimen. Hay un test que lo fija sobre los 85 negocios. */
const ORIGEN_VIEJO_A_NUEVO = {
  BDR: "B.d.r.",
  "Referido - BDR": "Ref. B.d.r.",
  "Referido - Martín": "Ref. Martin",
  "Referido - RE/MAX": "Ref. Remax",
  "Referido - Team": "Ref. Team",
  "Referido - cliente": "Ref. Cliente",
  "Redes pagas": "Redes sociales Campaña",
};

export const normalizarOrigen = (origen) => ORIGEN_VIEJO_A_NUEVO[origen] || origen || null;

export const esOrigenDeReferido = (origen) =>
  Boolean(origen) && (origen in ORIGEN_A_REGIMEN || ORIGENES_QUE_SON_REFERIDO.has(origen));
