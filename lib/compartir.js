...
export async function mandarArchivo(blob, nombre, texto = "", { telefono = "" } = {}) {
  ...
  if (puedeCompartirArchivos(archivo)) {
    /* El texto va JUNTO con el archivo cuando se puede: WhatsApp lo usa de pie de foto y el
       que lo recibe entiende que es sin tener que abrirlo. No todos los destinos aceptan las
       dos cosas a la vez, asi que se pregunta antes. */
    const conTexto = texto && typeof navigator.canShare === "function"
      && navigator.canShare({ files: [archivo], text: texto });
    try {
      await navigator.share(conTexto ? { files: [archivo], text: texto } : { files: [archivo] });
      return "compartido";
    } catch (error) {
      if (error && error.name === "AbortError") return "cancelado";
    }
