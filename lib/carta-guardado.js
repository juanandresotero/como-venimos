...
export function leerBorrador(almacen) {
  ...
  return {
    id: typeof guardado.id === "string" ? guardado.id : "",
    ...
    mandadas: comoMapa(guardado.mandadas),
    vueltas: comoMapa(guardado.vueltas),
    entregada: typeof guardado.entregada === "string" ? guardado.entregada : null,
  };
}
...
export function guardarBorrador(estado, cuando, almacen) {
  ...
  guardarJSON(BORRADOR, {
    ...
    firmas,
    cuando: cuando || null,
    mandadas: estado.mandadas || {},
    vueltas: estado.vueltas || {},
    entregada: estado.entregada || null,
  }, almacen);
}
...
export function leerHistorial(almacen) {
  ...
  return guardado.filter(esObjeto).map((c) => ({
    ...
    mandadas: comoMapa(c.mandadas),
    vueltas: comoMapa(c.vueltas),
    entregada: typeof c.entregada === "string" ? c.entregada : null,
  }));
}

/* Una carta puntual, por su numero. Es lo que se usa cuando vuelve una parte firmada: la
   vuelta trae el numero adentro del enlace y tiene que caer en SU carta, no en la que
   este abierta. */
export function leerDelHistorial(id, almacen) {
  if (!id) return null;
  return leerHistorial(almacen).find((c) => c.id === id) || null;
}
...
export function guardarEnHistorial(carta, cuando, almacen) {
  ...
  lista.unshift({
    ...
    /* A quien se le mando, quien contesto y si ya se les entrego la final. Es lo que
       distingue una carta en transito de un borrador. */
    mandadas: comoMapa(carta.mandadas),
    vueltas: comoMapa(carta.vueltas),
    entregada: carta.entregada || null,
  });
