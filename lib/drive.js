...
/* LA DIRECCION Y NADA MAS. Le tenia la fecha adelante y Juan la saco: "google drive ya guarda
   ese dato". Es su Drive y es su decision.

   OJO CON LA CONSECUENCIA, que esta dicha a proposito: si hace dos inventarios de la MISMA
   direccion —el mismo apartamento alquilado dos veces— los dos van a la misma carpeta y las
   fotos se mezclan. Para que sean dos, hay que escribir la direccion distinta.
...
export function comoSeLlamaLaCarpeta(inventario) {
  ...
  const nombre = [inv.direccion, inv.unidad ? `apto ${inv.unidad}` : ""]
