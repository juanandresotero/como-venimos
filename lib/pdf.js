...
export function nuevo({ ancho = A4.ancho, alto = A4.alto } = {}) {
  ...
  const doc = {
    ...
    /* VOLVER A UNA HOJA YA DIBUJADA. Hace falta cuando un dato no se sabe hasta el final: el
       inventario dice "está formado por N hojas" en el medio, y N recién se sabe al terminar
       de dibujarlo todo. Sin esto habría que contar las hojas dos veces —una para saber
       cuántas son y otra para dibujarlas— y las dos cuentas se irían separando. */
    enHoja(numero) {
      if (numero >= 0 && numero < hojas.length) actual = numero;
      return doc;
    },
