...
export function nuevo({ ancho = A4.ancho, alto = A4.alto } = {}) {
  ...
  const doc = {
    ...
    texto(x, y, contenido, { negrita = false, tamano = 11, espacioExtra = 0 } = {}) {
      const cadena = comoCadenaPDF(contenido);
      const cabeza = `BT /${negrita ? "F2" : "F1"} ${numero(tamano)} Tf `
        + `${numero(espacioExtra)} Tw ${numero(x)} ${numero(alto - y)} Td `;
      hojas[actual].push({ cabeza, cadena, cola: " Tj ET" });
      return doc;
    },
    ...
    rectangulo(x, y, anchoRect, altoRect) {
      orden([numero(x), numero(alto - y - altoRect), numero(anchoRect), numero(altoRect),
        "re", "f"]);
      return doc;
    },
    ...
    bytes() {
      return escribir(hojas, imagenes, ancho, alto);
    },

    aBlob() {
      return new Blob([escribir(hojas, imagenes, ancho, alto)], { type: "application/pdf" });
    },
  };
...
function escribir(hojas, imagenes, ancho, alto) {
  const salida = [];
...
<< /Type /XObject /Subtype /Image `
  ...
  const cuantos = primeraImagen + imagenes.length;
