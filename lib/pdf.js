...
export function nuevo({ ancho = A4.ancho, alto = A4.alto } = {}) {
  ...
  const doc = {
    ...
    bytes() {
      return escribir(hojas, ancho, alto);
    },

    aBlob() {
      return new Blob([escribir(hojas, ancho, alto)], { type: "application/pdf" });
    },
  };
...
function escribir(hojas, ancho, alto) {
  const salida = [];
  ...
  const cuantos = primeraHoja + hojas.length * 2;
