...
function marcarTinta({ data, width, height }) {
  /* Un PNG ya recortado, con fondo transparente, no necesita que le adivinemos nada: la
     ...
     imagen asi los dos metodos de abajo fallan — el fondo transparente no tiene color y
     el de brillo lo lee como si fuera negro. */
  const transparente = new Uint8Array(width * height);
  let opacos = 0;
  let translucidos = 0;
  for (let i = 3, p = 0; p < transparente.length; p++, i += 4) {
    if (data[i] > 128) {
      transparente[p] = 1;
      opacos++;
    }
  ...
  const tieneTransparencia = translucidos > width * height * 0.05;
  if (tieneTransparencia && opacos >= MINIMO_PIXELES) {
