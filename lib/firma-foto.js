...
function limpiar(tinta, ancho, alto, minimo, orilla = null) {
  ...
  const adentro = grupos.filter((g) => !g.tocaElBorde);
  const pixelesAdentro = adentro.reduce((n, g) => n + g.grupo.length, 0);
  const pixelesTotales = grupos.reduce((n, g) => n + g.grupo.length, 0);

  /* Si sacando los del borde queda menos de una quinta parte, la foto vino recortada al ras
     y eso que toca el borde era la firma. */
  if (pixelesAdentro * 5 < pixelesTotales) return pintar(grupos);
  return pintar(adentro);
}
