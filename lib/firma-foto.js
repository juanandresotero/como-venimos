...
/* La mascara se guarda ADENTRO DE ESTA CAJA, no con un ancho fijo.

   Antes solo se fijaba el ancho en 300. Con una firma ancha y baja daba una mascara chica,
   pero con una firma ALTA —una rubrica en diagonal, por ejemplo— daba 300 x 377, catorce mil
   bytes, y el enlace que el cliente devuelve por WhatsApp pasaba de 194 a mas de mil
   caracteres. Juan aviso que uno le llego cortado por la mitad, y un enlace cortado es una
   carta que no vuelve.

   La caja se elige por lo que hace falta: en el documento la firma se dibuja en un recuadro
   de 170 x 40 puntos, asi que mas resolucion que esta no se ve. Se achica lo que haga falta
   para entrar, siempre conservando la proporcion. */
export const ANCHO_GUARDADO = 300;
export const ALTO_GUARDADO = 110;
...
export function recortar(imagen, { alfaEsRecorte = false } = {}) {
  ...
  /* Se achica por el lado que se pase: si la firma es ancha manda el ancho, y si es alta
     manda el alto. Nunca se agranda, para no inventar detalle que la foto no tiene. */
  const encoger = Math.min(1, ANCHO_GUARDADO / anchoRecorte, ALTO_GUARDADO / altoRecorte);
  const anchoFinal = Math.max(1, Math.round(anchoRecorte * encoger));
  const altoFinal = Math.max(1, Math.round(altoRecorte * encoger));
