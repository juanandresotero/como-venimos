...
function marcarTinta(imagen, alfaEsRecorte) {
  ...
  const cuantosValidos = foto ? foto.cuantos : total;
  if (cuantosValidos < MINIMO_PIXELES) return null;

  const { tinta, cuantos } = porUmbralLocal(
    aGrisesConContraste(imagen, valido), width, height, valido,
  );
  ...
  return { tinta, porBrillo: cuantos > cuantosValidos * DEMASIADO };
}
