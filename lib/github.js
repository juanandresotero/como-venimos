...
/* ---------- Mandar al robot a mirar la cartera ---------- */

/* El robot corre solo una vez por dia a las 6 de la mañana. Esto lo manda a mirar AHORA.

   Es un `workflow_dispatch` de GitHub Actions: la app no puede correr el robot —es Python y
   necesita las credenciales de RE/MAX— pero si puede pedirle a GitHub que lo corra.

   OJO CON EL PERMISO: esto necesita `Actions: Read and write` en el token, que es un permiso
   MAS del que hace falta para guardar. Un token viejo va a dar 403, y por eso el error se
   explica en criollo en vez de dejar un numero suelto. */
const ARCHIVO_DEL_ROBOT = "robot.yml";

export async function mandarAlRobot(token) {
  try {
    const respuesta = await fetch(
      `https://api.github.com/repos/${REPO}/actions/workflows/${ARCHIVO_DEL_ROBOT}/dispatches`,
      {
        method: "POST",
        headers: { ...cabeceras(token), "Content-Type": "application/json" },
    ...
    if (respuesta.status === 204) return { ok: true };
    if (respuesta.status === 403 || respuesta.status === 404) {
      return {
        ok: false,
        mensaje: "Tu llave no tiene permiso para mandar al robot. En GitHub, editá el token y "
          + "agregale Actions: Read and write.",
      };
    }
    const cuerpo = await respuesta.json().catch(() => ({}));
    return { ok: false, mensaje: explicar(respuesta.status, cuerpo) };
  } catch {
    return { ok: false, mensaje: "No hay señal para hablar con GitHub." };
  }
}

/* Si el robot todavia esta corriendo. Sirve para saber cuando volver a bajar los datos.

   Se mira la ultima corrida del robot, no cualquiera: en el repo tambien corren las pruebas
   con cada cambio de codigo, y esas no traen datos nuevos. */
export async function comoVaElRobot(token) {
  try {
    const respuesta = await fetch(
      `https://api.github.com/repos/${REPO}/actions/workflows/${ARCHIVO_DEL_ROBOT}/runs?per_page=1`,
      { headers: cabeceras(token) }
    );
    if (!respuesta.ok) return { ok: false };
    const cuerpo = await respuesta.json();
    const [ultima] = cuerpo.workflow_runs || [];
    if (!ultima) return { ok: true, terminada: true };
    return {
      ok: true,
      terminada: ultima.status === "completed",
      salioBien: ultima.conclusion === "success",
