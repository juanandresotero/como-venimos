...
export const CAMPOS = [
  ...
  { clave: "sena", etiqueta: "Seña simbólica (U$S)", tipo: "monto", quien: "usuario", quitable: true, rayita: 18,
    pista: "Siempre en dólares. La carta lo aclara sola. Si no hay seña, quitá la casilla." },
...
export const PLANTILLA = [
  ...
  { tipo: "parrafo", piezas: [
    ...
    /* La seña se lleva la frase entera: si se quita la casilla, desaparece el pedazo
       completo y el SEGUNDO termina donde terminaba antes. */
    { campo: "sena",
      antes: " En este acto la parte OFERENTE entrega en concepto de seña simbólica la suma de ",
      despues: " dólares estadounidenses, que se imputará al precio de la compraventa proyectada." },
  ] },
...
export function armar(valores, quitadas = [], opciones = {}) {
  ...
  return PLANTILLA.map((bloque) => {
    ...
    for (const pieza of bloque.piezas) {
      ...
      /* Lo que va DESPUES tambien viaja con la casilla. Sin esto, quitar la seña dejaba
         colgado "dolares estadounidenses, que se imputara al precio" — media frase
...
/* Juntar lo que devolvio una parte con lo que ya tenias.

   Contesta una pregunta concreta del usuario: "¿que pasa si le mando la carta a las dos
   partes al mismo tiempo?".

   Sin esto, nada: cada enlace lleva su propia copia y las dos vuelven distintas, y habria
   que copiar a mano de una a la otra. Con esto se pueden mandar las dos a la vez, porque
   LAS CASILLAS DE CADA PARTE SON DISTINTAS: el comprador llena nombre, cedula, telefono y
   correo; el propietario llena las suyas de la segunda hoja. No se pisan nunca.

   Por eso de lo que vuelve se toma SOLO lo que le tocaba llenar a esa parte y su firma.
   Todo lo demas se ignora, aunque venga: si el comprador tocara sin querer el precio, ese
   cambio no entra. El precio lo pone el agente. */
export function fundir(base, entrante, turno) {
  const mias = new Set(CAMPOS.filter((c) => c.quien === turno).map((c) => c.clave));
  const FIRMA_DEL_TURNO = { comprador: "oferente", propietario: "propietario" };

  const valores = { ...(base.valores || {}) };
  for (const [clave, valor] of Object.entries((entrante.valores || {}))) {
    if (mias.has(clave) && valor !== null && valor !== undefined && valor !== "") {
      valores[clave] = valor;
    }
  }

  const firmas = { ...(base.firmas || {}) };
  const cual = FIRMA_DEL_TURNO[turno];
  const suya = (entrante.firmas || {})[cual];
  if (cual && suya && suya.length) firmas[cual] = suya;

  return { ...base, valores, firmas };
}

/* Que le toca llenar a cada parte. Lo usan la pantalla del cliente y `fundir`. */
export const CAMPOS_DE = (turno) => CAMPOS.filter((c) => c.quien === turno);
