...
/* Cada monto en su moneda. El motor calcula todo en la moneda del precio (dolares), pero
   lo que sale del alquiler se muestra como el usuario lo cargo: en Uruguay lo habitual es
   precio en dolares y alquiler en pesos, y decirle "cobras USD 469 por mes" a alguien que
   alquila en pesos lo obliga a hacer la cuenta de cabeza para saber si esta bien. */
const enPesos = (e) => e.moneda_alquiler === "UYU" && Number(e.tipo_cambio) > 0;
const delAlquiler = (usd, e) =>
  (enPesos(e) ? `$ ${monto(usd * e.tipo_cambio)}` : `USD ${monto(usd)}`);
...
export function contenido(entradas, resultados, agente = {}, opciones = {}) {
  ...
  if (mostrar !== "bruta") {
    ...
    if (gastosDelAnio) {
      filas.push(["Gastos del año (refacción, comisión, tributos)", delAlquiler(gastosDelAnio, e)]);
    }
  ...
  return {
    ...
    remate: mostrar === "bruta"
      ? [["Alquiler en el año", delAlquiler(r.renta_bruta_anual, e)]]
      : [
        ["Al bolsillo por mes", delAlquiler(r.bolsillo_por_mes, e)],
