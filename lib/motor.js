...
/* LOS AVISOS QUE SE CONTESTAN CON "ESTA BIEN", y que campo apagan.

   Son los dos casos donde la app no puede decidir sola pero tampoco quedarse callada: lo que
   hizo por su cuenta, y un cierre que el portal todavia no confirma. Vive aca —donde nacen
   los avisos— para que la bandeja y la ficha no puedan discrepar. */
export const PIDEN_VISTO_BUENO = {
  lo_hice_solo: "visto_bueno",
  firma_inventada: "firma_confirmada",
};
...
/* LO QUE SE BORRA AL CAERSE QUEDA GUARDADO, por si el negocio revive.

   Borrarlo esta bien: mientras ese precio y esa comision esten puestos, la proyeccion cuenta
   una plata que ya no va a entrar. Pero Juan toco "Sigue en marcha" en Minas 1600 y no le
   volvio nada: el precio, la comision, las puntas y quien trajo al comprador los tuvo que
   cargar de nuevo a mano. Ahora se guarda una copia al caerse y vuelve sola al revivir.

   Solo se guarda lo que de verdad CAMBIA. Sin eso, un caido ya limpio pisaba su propia copia
   en cada arranque con lo unico que le quedaba puesto (`puntas_confirmadas: false`). */
export function alCaerse(negocio, esUnaBusqueda) {
  const limpio = sinLoDeLaNegociacion(negocio, esUnaBusqueda);
  const guardado = {};
  for (const campo of Object.keys(limpio)) {
    const valor = negocio[campo];
    if (valor === null || valor === undefined || valor === limpio[campo]) continue;
    guardado[campo] = valor;
  }
  return {
    ...limpio,
    lo_borrado_al_caerse: Object.keys(guardado).length
      ? guardado
      : negocio.lo_borrado_al_caerse || null,
  };
}

/* Y AL REVIVIR VUELVE, pero sin pisar nada: si mientras estaba caido se cargo otro precio,
   manda el nuevo. La copia se usa una vez y se tira.

   OJO: esto es para cuando lo revive EL, con "Sigue en marcha" — o sea, cuando dice que el
   negocio nunca se cayo. Si el que lo revive es el portal (la propiedad volvio a negociacion)
   es otro comprador y arranca limpio: ahi la copia se tira sin usar. */
export function alRevivir(negocio) {
  const guardado = negocio.lo_borrado_al_caerse || {};
  const vuelve = {};
  for (const [campo, valor] of Object.entries(guardado)) {
    const ahora = negocio[campo];
    if (ahora === null || ahora === undefined) vuelve[campo] = valor;
  }
  return { ...vuelve, lo_borrado_al_caerse: null };
}
...
export function revisar(negocio, ajustes, hoy, cartera) {
  ...
  if (n.estado !== "cerrado" && !n.estado_a_mano && !n.fecha_fin) {
    ...
    if (alMercado && !estaCaido(n)) {
      n.estado = CAIDO;
      n.se_cayo_solo = true;
      n.revivio_solo = false;
      n.fecha_caida = n.fecha_caida || hoy;
      // Un cambio nuevo hecho por la app vuelve a pedir el visto bueno.
      n.visto_bueno = false;
      ...
      n.revivio_solo = true;
      n.visto_bueno = false;
      /* ARRANCA LIMPIO. Si la propiedad volvio a negociacion es porque entro OTRO comprador:
         el precio, las puntas y el agente de la negociacion anterior no son los de esta. La
         copia de lo que se borro al caerse se tira aca — solo sirve cuando el que revive el
         negocio es Juan diciendo "sigue en marcha", que es decir que nunca se cayo. */
      n.lo_borrado_al_caerse = null;
    }
  }

  /* UN CAIDO NO TIENE DATOS DE ESA NEGOCIACION — lo haya marcado el o lo haya visto la app.

     Antes limpiaba solo cuando lo detectaba sola. Juan marco a mano Juana de Ibarbourou y se
     encontro con la fecha de negociacion, el precio y las puntas todavia puestos: "en la app
     no se hicieron todos los cambios correspondientes" (2026-09-24). Es la misma cosa dicha
     de dos maneras, asi que ahora limpia igual, y lo que borra queda guardado por si revive. */
  if (estaCaido(n)) Object.assign(n, alCaerse(n, esBusqueda(n, ajustes)));
  else if (n.lo_borrado_al_caerse) Object.assign(n, alRevivir(n));
  ...
  if (!n.fecha_fin) {
    ...
    /* LA FIRMA ESCRITA EN PASADO ES PLATA COBRADA, y el estado tiene que decirlo.

       Faltaba justo el caso normal. Solo se cerraba una firma "estimada" —la que se escribio
       cuando todavia era futura—, y una firma real, puesta a mano o con el boton de "se
       vendio", dejaba el negocio EN CURSO para siempre. Lo cobrado solo cuenta los cerrados,
       y lo reservado solo las propiedades que siguen en el portal, asi que esa plata no
       estaba en ningun lado.

       Paso de verdad con San Fructuoso (2026-09-11): Juan contesto que se vendio y "la app no
       marca la diferencia economica". Eran USD 2.427 que no sumaban en ninguna capa.

       Manda aunque la propiedad siga en el portal: RE/MAX tarda dias en bajar una
       publicacion, y una firma escrita en pasado es una correccion explicita. */
    n.estado = "cerrado";
  }

  /* UNA PROPIEDAD QUE SE FUE DEL PORTAL ES UNA SOLA NOTICIA, y se cuenta en dos pasos.

     Primero la pregunta: ¿se vendio o se cayo? Mientras no se conteste es LO UNICO que se
     dice. Antes llegaban tres renglones con la misma noticia en otras palabras —esa pregunta,
     "hay datos nuevos para cargar" y "sin fecha de firma"— y una tarjeta mas por la
     propiedad. Juan los conto: "me salieron como 3 notificaciones del negocio cuando tendria
     que aparecer 1 solo". Los otros eran consecuencias: contestando, se ponen solos.

     Despues, revisar y dar la ficha por completa. La plata ya cuenta desde que contesto; el
     aviso solo espera a que diga que termino — "cuando este todo ok poner ficha completa o
     algo asi y ahi termine". */
  const seFue = Boolean(propiedad && !propiedad.activa);
  const porCerrar = seFue && n.estado !== "cerrado" && !estaCaido(n);
  const cobradoSinRevisar = seFue && n.estado === "cerrado" && !n.ficha_vigente;

  if (cobradoSinRevisar) {
    n.avisos.push(aviso("cierre_por_revisar",
      `Ya cuenta como cobrado desde el ${n.fecha_fin}. Revisá que esté todo bien y tocá `
      + "\"Ficha completa\"."));
  } else if (marcaVencida && !porCerrar && !estaCaido(n)) {
    /* A un caido tampoco: no se le pide nada —su ficha lo dice—, asi que "hay datos nuevos
       para cargar" era mentira. Llegaba pegado a "lo di por caido" (Minas 1600, 2026-09-11).

       Aca la propiedad sigue en el portal: si se hubiera ido, es uno de los dos casos de
       arriba o un caido. */
    n.avisos.push(aviso("ficha_reabierta",
  ...
  if (porCerrar) {
    /* AUNQUE LA FICHA ESTE DADA POR COMPLETA. Sin contestar, esa plata no cuenta en ningun
       lado, y "ficha completa" quiere decir "ya cargue lo que hay para cargar", no "ya se
       vendio". Si la marca la apagara, tocar ese boton antes de contestar dejaba la venta sin
       sumar para siempre y sin nadie que la reclame. Se vio probando, el 2026-09-11.

       UNA SOLA PREGUNTA: ¿se vendio o se cayo? Lo demas la app lo sabe — la fecha de firma
       es el dia que dejo de aparecer, y lo que cobraste sale del precio de cierre que ya
       cargaste cuando paso a negociacion. Las dos quedan editables por si un cierre
       importante cayo corrido de mes.

       Con SUS palabras: "se vendio o se cayo". "Concretar" no lo dice nadie. */
    const desenlace = propiedad.desenlace_confirmado || propiedad.desenlace_propuesto;
    const bien = n.tipo_negocio === "alquiler" ? "Se alquiló" : "Se vendió";
    n.avisos.push(aviso("cerrar_negocio",
      ...
      + `. ¿${bien} o se cayó?`));
  } else if (!n.ficha_vigente && !estaCaido(n)) {
  ...
  /* LO QUE LA APP HIZO SOLA ESPERA EL VISTO BUENO.

     Juan lo pidió así: "está bueno que sea todo lo más automático posible... que la app haga
     el cambio, pero que quede —si es un negocio caído o al revés— en avisos para yo darle el
     check bueno". La app sigue decidiendo sola, que es la gracia, pero no a escondidas: lo
     que hace queda en Hoy hasta que él diga que está bien.

     Antes el aviso de "lo di por caído" NO SE APAGABA NUNCA —no había con qué— y el de
     revivir ni existía: el negocio volvía a contar plata sin que nada lo dijera. */
  /* Como se dice cada estado en una frase, que no es como se escribe en el dato:
     "paso a en_negociacion" no lo dice nadie. */
  const COMO_SE_DICE = {
    en_negociacion: 'pasó a negociación',
    reservada: 'quedó reservada',
  ...
  if (!n.visto_bueno) {
    if (estaCaido(n) && n.se_cayo_solo) {
      ...
      n.avisos.push(aviso("lo_hice_solo",
        ...
        + "negociación, y te los repongo si me decís que sigue en marcha."));
    } else if (n.revivio_solo && !estaCaido(n)) {
      n.avisos.push(aviso("lo_hice_solo",
        `La propiedad volvió a estar ${momento === "reservada" ? "reservada" : "en negociación"} `
        + "en RE/MAX, así que puse este negocio otra vez en curso y vuelve a sumar. Lo de la "
        + "negociación anterior no volvió: ese comprador ya no es el de ahora."));
    } else if (n.nacio_solo) {
      n.avisos.push(aviso("lo_hice_solo",
        `Este negocio lo abrí yo${COMO_SE_DICE[momento]
          ? `, porque la propiedad ${COMO_SE_DICE[momento]} en RE/MAX` : ""}. `
        + "Revisá que esté bien."));
    }
  }

  /* DADO POR COBRADO, CON LA PROPIEDAD TODAVIA EN EL PORTAL.

     No se puede dar por mal: RE/MAX tarda dias en bajar un aviso y Juan firma antes — eso es
     lo normal, no un error. Pero tampoco por bien: una fecha de firma puesta por equivocacion
     deja plata contada como cobrada y saca la propiedad de lo reservado.

     Asi que se pregunta UNA vez y con el visto bueno se calla. Salio de Estanislao Vega
     (2026-09-24): "me decis que estanislao sigue esperandome pero no esta en avisos en la
     app" — la app se lo habia creido sin chistar. Se apaga solo el dia que el robot ve que la
     propiedad ya no esta publicada. */
  if (n.estado === "cerrado" && propiedad && propiedad.activa && !n.firma_confirmada) {
    n.avisos.push(aviso("firma_inventada",
      `Lo diste por cobrado el ${n.fecha_fin} y RE/MAX todavía muestra la propiedad `
      + `${propiedad.estado === "reservada" ? "reservada" : "publicada"}. Si ya firmaste está `
      + "bien, el portal tarda en bajar el aviso. Si no, borrale la fecha de firma y vuelve a "
      + "contar como reservada."));
  }
  ...
  for (const viejo of negocio.avisos || []) {
    ...
    /* "Figura cobrado pero la propiedad sigue viva" ya no se conserva: lo regenera `revisar`
       mirando la cartera de HOY y con su visto bueno. El del importador miraba una foto de
       agosto y no se apagaba nunca. */
    if (viejo.tipo === "firma_inventada") continue;
    n.avisos.push(viejo);
  }
