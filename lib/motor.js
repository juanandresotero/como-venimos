...
export const ATAJOS = {
  ...
  suplencia: {
    ...
    /* La fecha se pone SOLA con el dia de la carga. Una suplencia se anota cuando ya paso:
       si la estas cargando es porque la visita se hizo y el negocio se cerro. */
    negocia_hoy: true,
  },
  suplencia_alquiler: {
    ...
    puntas: 0,
    lado: "ninguna",
    /* En un alquiler NO HAY NEGOCIACION: se firma y listo. La fecha que se pone sola es la
       de la reserva, no la de negociacion. */
    reserva_hoy: true,
  },
  yo_referi: {
...
export function plantillaNegocio(atajo, ajustes, hoy) {
  ...
  return {
    ...
    /* Algunas nacen con la fecha PUESTA en el dia de la carga, porque se anotan cuando ya
       pasaron: una busqueda se carga cuando aparecio el comprador, y una suplencia cuando la
       visita ya se hizo y el negocio se cerro. Queda editable: si se cargo tarde, se corrige. */
    fecha_negociacion: busqueda || molde.negocia_hoy ? hoy : null,
    /* Y en un ALQUILER no hay negociacion: se firma y listo. Ahi la que se pone es la
       reserva. */
...
export function revisar(negocio, ajustes, hoy, cartera) {
  ...
  if (fechaParaLaPlata >= CORTE && !n.plata_acordada) {
  ...
  if (estaCaido(n) && n.se_cayo_solo) {
    ...
    n.avisos.push(aviso("se_cayo_solo",
      ...
      + "Si sigue en marcha, abrilo y avisame."));
  }
