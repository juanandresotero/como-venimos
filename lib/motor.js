...
export function revisar(negocio, ajustes, hoy, cartera) {
  ...
  if (!n.ficha_vigente) {
    ...
    if (esBusqueda(n, ajustes) && !n.fecha_fin) {
      const donde = n.fecha_boleto ? "reservada"
        : n.fecha_negociacion ? "en negociación" : "abierta";
      n.avisos.push(aviso("busqueda_en_curso",
        `Estás del lado del comprador y está ${donde}. Cuando se firme, cargale la fecha.`));
    } else if (!enMarcha) {
  ...
  if (n.estado === "en_curso" && !n.puntas_confirmadas) {
