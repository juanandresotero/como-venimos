...
export function comoTexto(cuenta) {
  ...
  if (c.titular) lineas.push(`Titular: ${c.titular}`);
  if (c.banco) lineas.push(`Banco: ${c.banco}`);
  /* La moneda se aclara solo cuando hay dos cuentas cargadas. Con una sola, "(dólares)" no
     desambigua nada y hace mas largo justo el renglon que el cliente tiene que copiar bien. */
  const dos = Boolean(c.pesos && c.dolares);
  if (c.pesos) lineas.push(`N° de Cuenta${dos ? " (pesos)" : ""}: ${c.pesos}`);
  if (c.dolares) lineas.push(`N° de Cuenta${dos ? " (dólares)" : ""}: ${c.dolares}`);
