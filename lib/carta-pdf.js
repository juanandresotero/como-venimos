...
function botonDeFirma(doc, y, enlace) {
  const ancho = 230;
  const alto = 30;
  ...
  doc.rectangulo(x, y, ancho, alto, [0, 0.26, 1]);   // el azul de la app
  doc.texto(x + (ancho - anchoDe(texto, { negrita: true, tamano })) / 2, y + 20, texto,
    { negrita: true, tamano, color: "blanco" });
  doc.enlace(enlace, { x, y, ancho, alto });

  const pie = "…o imprimí esta hoja y firmala a mano.";
  doc.texto((A4.ancho - anchoDe(pie, { tamano: 8.5 })) / 2, y + alto + 13, pie, { tamano: 8.5 });
  return alto + 22;
}
...
/* `bloques` es lo que devuelve armar(); `firmas` son los bytes de cada firma hecha;
   ...
   direccion que abre la pantalla de firmar, si se quiere el boton. */
export function armarPDF(bloques, firmas = {}, membrete = null, enlaceParaFirmar = "") {
  ...
  /* El boton va al FINAL del documento, cuando ya se leyo todo. Se probo ponerlo despues
     de las firmas de la primera hoja y no entraba: empujaba una hoja de mas, y una carta
     oferta de tres hojas no es una carta oferta. */
  if (enlaceParaFirmar) {
    if (!cabeEn(72)) hojaNueva();
    y += 10;
    botonDeFirma(doc, y, enlaceParaFirmar);
  }
