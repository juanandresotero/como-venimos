...
export function derivar(negocios, eventos, hoy, cartera) {
  ...
  for (const negocio of negocios || []) {
    ...
    if (negocio.ficha_vigente ?? negocio.ficha_completa) continue;
  ...
  for (const propiedad of Object.values(cartera || {})) {
    if (!propiedad.activa || propiedad.estado !== "en_negociacion") continue;
    ...
    agregar("falta_precio_negociacion", {
      ...
      detalle: `Está en negociación${propiedad.fecha_negociacion ? ` desde el ${propiedad.fecha_negociacion}` : ""}`
...
/* Que se puede hacer con un pendiente.
   ...
   elegia el boton con un if/else de tres ramas, y un aviso del robot trae `evento_id`
   Y TAMBIEN `entity_id`. Como `evento_id` se preguntaba primero, esos avisos ofrecian
...
export function accionesDe(item) {
  ...
  if (item.evento_id) {
    acciones.push({ tipo: "atendido", destino: item.evento_id, texto: "Ya lo resolví" });
  }
  return acciones;
}
