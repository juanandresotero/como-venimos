...
import { PIDEN_VISTO_BUENO } from "./motor.js";
...
export const GRUPOS = {
  ...
  /* Ya contesto que se vendio y la plata ya cuenta. Queda revisar que este todo bien y dar la
     ficha por completa: es el final del trabajo con ese negocio, no algo urgente. */
  cierre_por_revisar: { nombre: "Ya cobrados: revisá y dalos por completos", orden: 0.5, urgente: false },
  ...
  /* LO QUE LA APP HIZO SOLA: dio un negocio por caido, lo revivio, o abrio uno nuevo. Va
     arriba y en rojo porque son cambios de PLATA hechos sin preguntar, y se apagan con el
     "Esta bien" — antes el de caerse no se apagaba nunca. Lo pidio Juan: "que la app haga el
     cambio, pero que quede en avisos para yo darle el check bueno". */
  lo_hice_solo: { nombre: "Cambios que hice solo — dale el visto bueno", orden: 2.6, urgente: true },
  ...
  /* Igual que una busqueda: no figura en ninguna otra pantalla salvo Cartera, y es lo unico
     que se puede olvidar sin que nada lo recuerde. */
  referida_en_curso: { nombre: "Propiedades que referiste", orden: 9.6, urgente: false },
  /* Una suplencia sin cobrar NO va a la bandeja: vive en Cartera, debajo de las busquedas y
     las referidas. Lo pidio Juan asi — "sacala de hoy y dejala solo en cartera" — porque
     estando en los dos lados la misma suplencia se cuenta dos veces y Hoy se llena de cosas
     que no hay que hacer HOY: una suplencia se cobra cuando el colega paga, no cuando la app
     lo recuerda.

     El aviso igual existe: se sigue viendo en "Qué falta acá" al abrir la ficha, que es
     donde vas cuando querés saber qué le falta a ESE negocio. Ver FUERA_DE_LA_BANDEJA. */
  suplencia_sin_cobrar: { nombre: "Suplencias sin cobrar", orden: 9.7, urgente: false },
  /* LO QUE PASA CON LO QUE REFERISTE. El robot le mira la cartera al colega —el no te cuenta
     como viene la cosa— y estos cuatro avisos son lo que encuentra. Van arriba de casi todo
     porque son plata en manos de otro: si no te enterás acá, no te enterás. */
  referida_candidata: { nombre: "¿Es la que le referiste?", orden: 4.1, urgente: false },
  referida_avanzo: { nombre: "Avanzó una que referiste", orden: 4.2, urgente: false },
  referida_se_fue: { nombre: "¿Cómo terminó la que referiste?", orden: 4.3, urgente: true },
  referida_cambio_precio: { nombre: "Le cambiaron el precio a una que referiste", orden: 9.8, urgente: false },
...
/* SOLO PARA MIRAR, no para hacer: cambios de precio, cambios de estado y reapariciones.

   El robot los detecta y quedan en el historial de la propiedad, pero NO van a la bandeja:
   ahi va lo que hay que hacer, y estos no piden nada. Juan: "que no me avise si hay cambios
   de precio, estado o reapariciones si es solo para chequeo".

   Lo que sí llega igual es lo que ESOS cambios destapan: si pasa a negociacion, aparece
   "falta a que precio" y "una punta o dos"; si vuelve al mercado, el negocio se da por caido
   y se avisa. El cambio en si no hace falta contarlo dos veces. */
const SOLO_PARA_MIRAR = new Set(["cambio_precio", "cambio_estado", "reaparecio"]);

/* Avisos de un negocio que se ven en su ficha pero NO en la bandeja de Hoy.

   Hoy es "qué hago hoy". Una suplencia sin cobrar no se resuelve hoy: se cobra el día que el
   colega paga. Y ya tiene su lugar en Cartera, debajo de las búsquedas y las referidas, que
   es donde Juan la buscó. Tenerla en los dos lados es contar lo mismo dos veces. */
const FUERA_DE_LA_BANDEJA = new Set(["suplencia_sin_cobrar"]);
...
/* "¿Se vendio o se cayo?" tambien: sin contestarla, esa venta no suma en ningun lado. Y lo
   que la app hizo sola, por la misma razon: no es un dato que falte, es plata que se movio. */
const ATRAVIESAN_FICHA_COMPLETA = new Set(
  ["revisar_puntas", "lo_hice_solo", "comision_absurda", "cerrar_negocio", "firma_inventada"]);

function comoSeLlama(negocio) {
  return negocio.direccion || negocio.barrio || `Negocio ${negocio.id}`;
}

/* Los eventos que TODAVIA hay que atender.

   Vivia copiada en dos lados —`app.js` para el globito del contador y `vistas/hoy.js` para la
   lista— y eso ya se cobro una: al cambiar la regla en uno, el otro siguio con la vieja y la
   bandeja mostraba una cosa distinta del numero de al lado. Ahora esta una sola vez.

   LOS QUE PIDEN EL ORIGEN VUELVEN si el origen sigue vacio. Despachar uno de esos con "no lo
   voy a cargar" apaga el aviso sin resolver nada, y de ese dato sale la regla de comision — o
   sea, la plata que proyecta esa propiedad.

   Paso de verdad: Juan despacho quince avisos y dos quedaron sin origen. Nadie se lo iba a
   pedir nunca mas, y esas dos seguian proyectando con una regla inventada.

   Si la propiedad ya no esta activa, el dato dejo de importar y no se insiste. */
const PIDEN_EL_ORIGEN = new Set(["alta", "carga_inicial", "origen_sin_clasificar"]);

export function sinAtender(eventos, misDatos, cartera) {
  const atendidos = new Set((misDatos || {}).eventos_atendidos || []);
  const filas = cartera || {};
  return (eventos || []).filter((e) => {
    if (!atendidos.has(e.id)) return true;
    if (!PIDEN_EL_ORIGEN.has(e.tipo)) return false;
    const propiedad = filas[e.entity_id];
    return Boolean(propiedad && propiedad.activa && !propiedad.origen_captacion);
  });
}
...
export function derivar(negocios, eventos, hoy, cartera) {
  ...
  const agregar = (clave, item) => {
    ...
    /* El item se lleva su grupo: sin eso, `accionesDe` no puede saber si este aviso pide un
       dato o es una noticia, y el boton diria lo mismo en los dos casos. */
    item.clave = clave;
    if (!mapa.has(clave)) {
      mapa.set(clave, {
        clave,
  ...
  for (const negocio of negocios || []) {
    /* Un caido no es un negocio abierto: no puede hacer que otro figure como duplicado. */
    if (!negocio.entity_id_cartera) continue;
    if (negocio.estado === "cerrado" || negocio.estado === "caido") continue;
  ...
  for (const negocio of negocios || []) {
    ...
    for (const aviso of negocio.avisos || []) {
      if (FUERA_DE_LA_BANDEJA.has(aviso.tipo)) continue;
      agregar(aviso.tipo, {
        negocio_id: negocio.id,
        titulo: comoSeLlama(negocio),
        detalle: aviso.detalle,
  ...
  for (const evento of eventos || []) {
    ...
    /* Los que son sólo para mirar no entran: quedan en el historial de la propiedad y en la
       pantalla de la ficha, pero la bandeja es lo que hay que HACER. */
    if (SOLO_PARA_MIRAR.has(evento.tipo)) continue;
    /* LA BAJA DE UNA PROPIEDAD CON UN NEGOCIO ABIERTO YA LA PREGUNTA EL NEGOCIO. Es la misma
       pregunta —¿se vendio o se cayo?— y llegaba en dos tarjetas (San Fructuoso,
       2026-09-11). Al contestarla en el negocio, la baja queda despachada sola: ver
       `editarPropiedad`. Sin negocio abierto, o con uno ya cerrado de otra vuelta, la baja
       es la unica que pregunta y se queda. */
    if (evento.tipo === "baja" && abiertosPorPropiedad.has(evento.entity_id)) continue;
    ...
    agregar(evento.tipo, {
      ...
      /* Los avisos de una REFERIDA no hablan de una propiedad de tu cartera sino de un
         NEGOCIO tuyo en la cartera de otro. Sin esto el botón no sabría a qué ficha ir. */
      negocio_id: evento.negocio_id || null,
...
/* Los avisos que PIDEN un dato concreto. En estos, "Ya lo resolví" es una mentira posible:
   apaga el aviso aunque el dato siga faltando, y despues nadie lo vuelve a pedir nunca.

   Paso de verdad: Juan despacho quince avisos, y dos de ellos —Juana de Ibarbourou 200 y
   Minas 1600— quedaron sin el origen de la captacion. De ese dato sale la regla de comision,
   asi que esas dos propiedades proyectaban plata con una regla inventada y la app ya no se lo
   iba a pedir mas.

   Para estos, el boton dice lo que de verdad hace. */
const PIDEN_UN_DATO = new Set(["alta", "carga_inicial", "origen_sin_clasificar"]);

export function accionesDe(item) {
  /* LOS QUE SE CONTESTAN CON "ESTA BIEN": lo que la app hizo sola, y un cierre que el portal
     todavia no confirma. En los dos el hecho YA ocurrio —no es un dato que falte— asi que el
     boton que resuelve no abre nada, lo aprueba. Abrirlo queda de segunda, por si lo quiere
     mirar o deshacer desde la ficha. */
  const campo = PIDEN_VISTO_BUENO[item.clave];
  if (campo && item.negocio_id) {
    return [
      { tipo: "visto-bueno", destino: item.negocio_id, campo, texto: "Está bien" },
      { tipo: "ficha", destino: item.negocio_id, texto: "Abrir" },
    ];
  }
  ...
  /* UN DUPLICADO SE CONTESTA CON SI O NO, y de eso depende la plata que proyecta.

     El robot, al detectarlo, ya lo saca de la proyeccion: es lo prudente, porque contar dos
     veces la misma propiedad infla los numeros. Pero si son DOS distintas —pasa: dos unidades
     en el mismo edificio al mismo precio— hay que poder devolverla. */
  /* "¿ES LA QUE LE REFERISTE?" SE CONTESTA CON SI O NO, y nada más — el que sabe es el
     colega, así que Juan le pregunta y vuelve con la respuesta.

     Con un SÍ quedan enganchadas y de ahí en adelante la app le sigue el rastro sola. Con un
     NO esa propiedad queda descartada para siempre: preguntar dos veces lo mismo es hacerle
     llamar al colega dos veces. */
  if (item.clave === "referida_candidata" && item.negocio_id && item.entity_id) {
    return [
      { tipo: "es-la-referida", destino: item, texto: "Sí, es esa" },
      { tipo: "no-es-la-referida", destino: item, texto: "No" },
    ];
  }

  if (item.clave === "posible_duplicado" && item.entity_id) {
    return [
      { tipo: "cuenta", destino: item.entity_id, texto: "Son dos distintas" },
      { tipo: "no-cuenta", destino: item.entity_id, texto: "Es la misma" },
    ];
  }
  if (item.eventos && item.eventos.length) {
    acciones.push({
      tipo: "atendido",
      destino: item.eventos,
      texto: PIDEN_UN_DATO.has(item.clave) ? "No lo voy a cargar" : "Ya lo resolví",
    });
  }
  return acciones;
}
...
function describirEvento(evento) {
  ...
  switch (evento.tipo) {
    ...
    /* Los del robot que miran la cartera del colega ya vienen con el texto escrito: ahí el
       detalle es una frase, no un objeto de datos, porque lo que hay que decir depende de lo
       que vio ese día y no de un molde fijo. */
    case "referida_candidata":
    case "referida_avanzo":
    case "referida_se_fue":
    case "referida_cambio_precio":
      return typeof d === "string" ? d : (evento.detalle || "");
    default:
