...
const ATRAVIESAN_FICHA_COMPLETA = new Set(
  ["revisar_puntas", "lo_hice_solo", "comision_absurda", "cerrar_negocio"]);
...
export function accionesDe(item) {
  /* LO QUE HICE SOLO SE CONTESTA CON "ESTA BIEN": es un cambio que YA ocurrio, no un dato que
     falte, asi que el boton que resuelve no abre nada — lo aprueba. Abrirlo queda de segunda,
     por si lo quiere mirar o deshacer desde la ficha. */
  if (item.clave === "lo_hice_solo" && item.negocio_id) {
    return [
      { tipo: "visto-bueno", destino: item.negocio_id, texto: "Está bien" },
