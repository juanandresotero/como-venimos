...
import { nuevo, A4, anchoDe } from "./pdf.js";
import { comoSeLee, conCantidad, numerar, comoSeLlama } from "./inventario.js";

const MARGEN = 42;
const ANCHO = A4.ancho - MARGEN * 2;
/* El pie lleva la dirección de la oficina, igual que el suyo. */
const PIE = 56;
const TOPE = A4.alto - PIE;
...
export function armarPDF(inventario, { oficina = "" } = {}) {
  const doc = nuevo();
  const inv = { ...inventario, ambientes: numerar(inventario.ambientes || []) };
  ...
  function pieDeHoja(numero) {
    hoja.linea(MARGEN, TOPE + 14, A4.ancho - MARGEN, TOPE + 14, 0.6);
    hoja.texto(MARGEN, TOPE + 30, oficina, { tamano: CHICO, color: GRIS });
    const n = `Hoja ${numero}`;
    hoja.texto(A4.ancho - MARGEN - anchoDe(n, { tamano: CHICO }), TOPE + 30, n,
  ...
  // ---------------------------------------------------------------- el encabezado
  hoja.texto(MARGEN, y + 16, "Inventario del inmueble", { negrita: true, tamano: TITULO });
  y += 26;
  ...
  const tituloDeAmbiente = (nombre) => {
    ...
    hoja.texto(MARGEN + PADDING, y + 14, nombre.toUpperCase(), { negrita: true, tamano: 10 });
    y += 20;
  };

  for (const ambiente of inv.ambientes) {
    const filas = (ambiente.items || [])
      .filter((i) => i.estado !== "no_tiene")
      .map((i) => [conCantidad(i), comoSeLee(i)]);
