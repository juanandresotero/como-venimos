...
export function armarPDF(inventario, { oficina = "", membrete = null, fotos = [] } = {}) {
  ...
  const altoDeLasFirmas = 30 + Math.ceil(Math.max(1, cuantasFirmas(inv.firmas_arrendador))
    / FIRMAS_POR_FILA) * ALTO_FIRMA
    + 30 + Math.ceil(Math.max(1, cuantasFirmas(inv.firmas_arrendatario))
      / FIRMAS_POR_FILA) * ALTO_FIRMA;
  ...
  const bloqueDeFirmas = (titulo, cuantas) => {
    const filas = Math.ceil(cuantas / FIRMAS_POR_FILA);
    espacio(22 + filas * ALTO_FIRMA);
    ...
    for (let i = 0; i < cuantas; i += 1) {
      ...
      hoja.linea(x, y + 26, x + anchoRaya, y + 26, 0.8);
      hoja.texto(x, y + 38, "Firma y aclaración", { tamano: CHICO, color: GRIS });
      if (columna === FIRMAS_POR_FILA - 1 || i === cuantas - 1) y += ALTO_FIRMA;
    }
    y += 10;
  };
