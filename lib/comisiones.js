...
export function facturaDeLaPunta(punta, { regimen, split, conIva }) {
  ...
  /* Lo que hubiera pagado sin el descuento, con el MISMO IVA encima.

     Es la cifra contra la que el cliente compara. Ojo con una intuición que parece cierta
     y no lo es: sumarle el IVA a las dos puntas no mueve el porcentaje de descuento. El
     IVA multiplica igual al número con descuento y al número sin descuento, así que se
     simplifica — un 23% de descuento sobre la comisión es 23% de descuento sobre el total,
     lleve IVA o no. Lo que sí cambia es la PLATA: sin IVA se ahorra 690 y con IVA 842, y
     esa diferencia es lo que hace que valga la pena mostrar los dos números. */
  const comision_lista = numero(punta.bruto, punta.neto);
  const total_lista = comision_lista * (1 + recargo);
...
/* Por debajo de esto no se nombra el descuento: es ruido de decimales, no un descuento. */
const DESCUENTO_QUE_SE_NOMBRA = 0.0005;

/* El renglón del descuento, para pegar abajo del total.

   El cliente lee "3% + IVA" y siente que el IVA es un extra que le cobran de más. Decirle
   "te hago un 23% de descuento" no le alcanza, porque no sabe sobre qué. Lo que entiende es
   un número final contra el otro número final: paga 2.818 en vez de 3.660. Con los dos
   totales a la vista puede verificar el porcentaje solo, y eso es lo que hace que se lo crea. */
function renglonDelDescuento(punta) {
  if (!(punta.pct_descuento > DESCUENTO_QUE_SE_NOMBRA)) return [];
  return [
    `Descuento aplicado: ${porciento(punta.pct_descuento)} — sin el descuento el total sería ${monto(punta.total_lista)}.`,
  ];
}
...
export function textoParaElCliente(punta, { precio, titulo, cuenta } = {}) {
  ...
  lineas.push(`Son: ${monto(punta.comision)}`);
  if (punta.iva) {
