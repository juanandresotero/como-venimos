...
export function textoParaElCliente(punta, { precio, titulo, agente } = {}) {
  ...
  /* Va la OFICINA y no el agente: este texto es el detalle de lo que se le cobra, y
     quien se lo manda ya esta hablando con el por WhatsApp. El nombre y el telefono ahi
     abajo no agregaban nada — el cliente los tiene en la pantalla del chat. */
  lineas.push("");
  lineas.push(`Oficina ${a.oficina || "RE/MAX Único"}`);
  return lineas.join("\n");
}
