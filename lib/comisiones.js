...
/* El renglón del descuento, para pegar abajo del total.
   ...
   Decirle "te hago un 23% de descuento" no le alcanza al cliente, porque no sabe sobre qué.
   Lo que entiende es un número final contra el otro número final: paga 2.818 en vez de
   3.660. Con los dos totales a la vista puede verificar el porcentaje solo, y eso es lo que
   hace que se lo crea.

   Se nombra la referencia completa —"el 3% + IVA"— porque es la frase que el cliente ya
   tiene en la cabeza y contra la que estaba midiendo. */
function renglonDelDescuento(punta) {
  if (!(punta.pct_descuento > DESCUENTO_QUE_SE_NOMBRA)) return [];
  const lista = punta.pct_lista ? `el ${porciento(punta.pct_lista)} + IVA` : "la cuenta completa";
  /* Un decimal y no tres: esto es una frase, no un contador. "18%" se lee, "18,033%" hace
     dudar de si hay letra chica. El monto exacto está en el renglón de arriba. */
  return [
    `Descuento aplicado: ${porciento(punta.pct_descuento, 1)} — sin descuento, ${lista} sería ${monto(punta.total_lista)}.`,
  ];
}
