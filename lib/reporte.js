...
export function armarReporte(datos, anio, hoy, { nombre = "Juan Andrés Otero", oficina = "RE/MAX Único" } = {}) {
  ...
  partes.push(`<section>
    ...
    <p class="heroe">${monto(c.capa1.ganancia)}</p>
    <p class="chico">a tu bolsillo · <b>${monto(c.capa1.facturacion)}</b> facturados
      · ${c.capa1.negocios} ${c.capa1.negocios === 1 ? "negocio" : "negocios"}</p>
    <p class="chico" style="margin-top:14px">Si cierra todo lo que está en negociación y
      reservado: <b>${monto(c.capa1.ganancia + c.avanzado.ganancia)}</b> a tu bolsillo,
      ${monto(c.capa1.facturacion + c.avanzado.facturacion)} facturados.</p>
  </section>`);
  ...
  const detalle2 = c.capa2.detalle.map((n) => n.direccion || n.barrio || n.id).filter(Boolean);
  const detalle3 = c.capa3.detalle.map((p) => p.direccion).filter(Boolean);
  partes.push(`<section>
    <h2>De dónde sale la plata</h2>
    <div class="capa"><div><b>Cobrado</b><p class="detalle">${c.capa1.negocios} negocios cerrados</p></div>
      <div style="text-align:right"><b>${monto(c.capa1.facturacion)}</b><p class="detalle">${monto(c.capa1.ganancia)} tuyos</p></div></div>
    <div class="capa"><div><b>Casi seguro</b><p class="detalle">${escapar(detalle2.join(", ") || "nada en curso")}</p></div>
      <div style="text-align:right"><b>${monto(c.capa2.facturacion)}</b><p class="detalle">${monto(c.capa2.ganancia)} tuyos</p></div></div>
    <div class="capa"><div><b>Potencial de la cartera</b><p class="detalle">${escapar(detalle3.join(", ") || "sin propiedades publicadas")}</p></div>
      <div style="text-align:right"><b>${monto(c.capa3.facturacion)}</b><p class="detalle">${monto(c.capa3.ganancia)} tuyos</p></div></div>
