...
import { momentoDelNegocio } from "./motor.js";
...
/* EL RESUMEN DE ARRIBA DE LA CARTERA: cuanto hay publicado, cuantas negociando y cuantas
   reservadas.

   UNA BUSQUEDA CUENTA EN EL MOMENTO EN QUE ESTA. Antes todas sumaban a "negociando" —"una
   busqueda abierta ESTA en negociacion"— y eso deja de ser cierto en cuanto se firma el
   boleto: Juan contaba 5 reservadas y la pantalla le mostraba 3. El cartelito de cada fila ya
   decia "Reservada"; el numero de arriba no lo miraba.

   Vive aca y no en la pantalla por la misma razon de siempre: una cuenta escrita en la vista
   no se puede probar, y esta ya se habia equivocado. */
export function comoVieneLaCartera(activas, busquedas) {
  const cuenta = (clave) => (activas || []).filter((p) => p.estado === clave).length;
  const conBoleto = (busquedas || [])
    .filter((n) => momentoDelNegocio(n) === "reservada").length;
  return {
    volumen: (activas || []).reduce((t, p) => t + (p.precio || 0), 0),
    negociando: cuenta("en_negociacion") + (busquedas || []).length - conBoleto,
    reservadas: cuenta("reservada") + conBoleto,
  };
}
