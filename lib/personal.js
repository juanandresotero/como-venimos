...
/* Las categorías con las que arranca. Cortas y pocas: en el momento de cargar un gasto en la
   calle, elegir entre veinte es no cargarlo.

   Pero son SOLO EL PUNTO DE PARTIDA. La lista es del usuario: puede agregar las suyas y sacar
   las que no usa. Una lista fija obliga a meter medio gasto en "Otros", y "Otros" con la
   mitad de la plata adentro no dice nada. */
export const CATEGORIAS_DE_FABRICA = [
  "Comida", "Casa", "Transporte", "Salidas", "Salud", "Ropa", "Otros",
];

/* Se sigue exportando con el nombre viejo para lo que sólo necesita las de arranque. */
export const CATEGORIAS = CATEGORIAS_DE_FABRICA;

/* Las que hay que ofrecer. Sin nada guardado, las de fábrica. */
export function categoriasDe(datos) {
  const propias = ((datos || {}).categorias || []).filter((c) => String(c || "").trim());
  return propias.length ? propias : [...CATEGORIAS_DE_FABRICA];
}

/* Agregar una. Se limpia el nombre y no se repite: dos categorías que sólo se distinguen por
   una mayúscula o un espacio son dos formas de perder la misma plata. */
export function agregarCategoria(datos, nombre) {
  const limpio = String(nombre || "").trim().replace(/\s+/g, " ");
  if (!limpio) return datos;
  const actuales = categoriasDe(datos);
  if (actuales.some((c) => c.toLowerCase() === limpio.toLowerCase())) return datos;
  return { ...datos, categorias: [...actuales, limpio] };
}

/* Sacar una. Nunca se queda sin ninguna: sin categorías no se puede cargar un gasto.

   Los gastos YA CARGADOS con esa categoría no se tocan: cada uno la lleva escrita adentro, y
   reescribir el pasado por un cambio de hoy es lo que hace que las gráficas de meses cerrados
   cambien solas. Simplemente deja de ofrecerse para los nuevos. */
export function sacarCategoria(datos, nombre) {
  const quedan = categoriasDe(datos).filter((c) => c !== nombre);
  if (!quedan.length) return datos;
  return { ...datos, categorias: quedan };
}

/* Cuántos gastos usan una categoría, para poder avisar antes de sacarla. */
export function cuantosUsan(datos, nombre) {
  return ((datos || {}).variables || []).filter((v) => v.categoria === nombre).length;
}
...
export const VACIO = {
  ...
  aprendidas: {},
  /* Vacío quiere decir "las de fábrica". Recién cuando el usuario la toca, se guarda. */
  categorias: [],
};
...
export function sanear(datos) {
  ...
  return {
    ...
    aprendidas: d.aprendidas && typeof d.aprendidas === "object" && !Array.isArray(d.aprendidas)
      ? d.aprendidas
      : {},
    categorias: lista(d.categorias).filter((c) => typeof c === "string" && c.trim()),
  };
}
  ...
  return (negocios || [])
    ...
    .map((n) => {
      /* LA PLATA ENTRA EN LA MONEDA EN QUE LA COBRASTE.
         ...
         El negocio cuenta en dólares —así te mide RE/MAX y así se arman Salud y el año— pero
         a tu cuenta entró lo que entró: si el alquiler se cobró en pesos, son pesos. Lo pidió
         Juan: "si cobro en pesos luego en la cara personal me ingresa como moneda pesos".

         El tipo de cambio es el que quedó guardado EN EL NEGOCIO, no el de hoy: un alquiler
         cobrado en marzo entró al dólar de marzo. */
      /* Sólo "UYU" cuenta como pesos. Un negocio SIN moneda es de los viejos, y todos esos
         estaban en dólares: si el que falta se leyera como pesos, la cuenta del bolsillo se
         iría a cualquier lado sin que nada avise. */
      const moneda = n.moneda === "UYU" ? "UYU" : "USD";
      const cambio = moneda === "UYU" ? Number(n.tipo_cambio) || 0 : 1;
      const bruto = numero(n.ganancia) * cambio;
      return {
        id: n.id,
        fecha: n.fecha_fin,
...
export function saldos(personal, negocios, hasta) {
  ...
  for (const c of cobrosDeNegocios(negocios, d.arranque.fecha, d.impuestos_pct)) {
    if (c.fecha <= tope) sumar(caja, c.moneda, c.monto);
  }
