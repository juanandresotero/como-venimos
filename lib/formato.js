...
/* Un porcentaje con HASTA tres decimales, sin ceros de relleno.

   "3%" se lee de un golpe y "3,000%" hace dudar de si hay algo escondido atras. Pero un
   descuento raro puede caer en 2,308% y ahi los tres decimales hacen falta: redondeando a
   dos, el numero que se le dice al cliente deja de coincidir con la plata. */
export function pctFino(n, maximo = 3) {
  if (!hayValor(n)) return "—";
  const valor = Number((n * 100).toFixed(maximo));
