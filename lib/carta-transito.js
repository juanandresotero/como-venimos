...
/* Qué firma le corresponde a cada parte adentro del documento. */
const FIRMA_DE = { comprador: "oferente", propietario: "propietario" };
...
/* A quién se está esperando todavía.

   Lo que decide es LA FIRMA EN EL DOCUMENTO, no quién tocó el enlace. Antes se miraba sólo
   si la parte había devuelto la carta, y eso mentía: si Juan completaba y firmaba una parte
   él mismo —o el cliente firmaba delante suyo—, el documento quedaba completo y el tablero
   seguía diciendo "esperando al comprador". Le pasó y no lo entendía.

   La vuelta por el enlace igual cuenta, por si algún día una parte devuelve algo sin firma. */
export const faltanVolver = (carta) => {
  const firmas = (carta || {}).firmas || {};
  return Object.keys(mandadas(carta))
    .filter((t) => !firmas[FIRMA_DE[t]] && !(t in vueltas(carta)));
};
...
/* Si el DOCUMENTO esta completo: firmaron las dos partes.

   Es distinto de `estaPronta`, que ademas pide que la carta este en el tablero. Una carta
   ya archivada sigue estando completa, y tiene que dejar mandarse igual — Juan quiso el
   mismo boton en el historial que en transito. */
export const tieneLasDosFirmas = (carta) => {
  const firmas = (carta || {}).firmas || {};
  return Boolean(firmas.oferente && firmas.propietario);
};
...
export function anotarMandada(carta, turno, cuando) {
  ...
  return {
    ...carta,
    id: carta.id || nuevoId(),
    /* Mandarla de nuevo REABRE el circuito: una carta archivada que se vuelve a mandar tiene
       que volver al tablero, porque otra vez se está esperando que alguien conteste. Sin
       esto quedaba archivada y no había dónde ver que estaba dando vueltas. */
    entregada: null,
...
/* Deshacer el archivado: la carta vuelve al tablero tal como estaba. Existe porque el
   boton de archivar es de un toque y no se puede quedar sin vuelta atras. */
export const devolverAlTablero = (carta) => ({ ...carta, entregada: null });
